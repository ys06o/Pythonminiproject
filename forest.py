...
# 임상도 파일
# 서울
df강1= pd.read_csv('임상도/강원특별자치도1.csv',
                 header=0,
...
df강2= pd.read_csv('임상도/강원특별자치도2.csv',
                 header=0,
                 usecols=['갱신년도','FRTP_CD','AGCLS_CD','DNST_CD','latitude','longitude','location'],                 
                 encoding='utf-8-sig'  # CSV 파일이 UTF-8 BOM 형식이라 utf-8-sig로 읽어야 함
                                   ) 
df강원특별자치도 = pd.concat( [df강1 , df강2 ], ignore_index=True )

df경기도= pd.read_csv('임상도/경기도.csv',
                 header=0,
                 usecols=['갱신년도','FRTP_CD','AGCLS_CD','DNST_CD','latitude','longitude','location'],                 
                 encoding='utf-8-sig'  # CSV 파일이 UTF-8 BOM 형식이라 utf-8-sig로 읽어야 함
                                   ) 

df경상남도= pd.read_csv('임상도/경상남도.csv',
                 header=0,
                 usecols=['갱신년도','FRTP_CD','AGCLS_CD','DNST_CD','latitude','longitude','location'],                 
                 encoding='utf-8-sig'  # CSV 파일이 UTF-8 BOM 형식이라 utf-8-sig로 읽어야 함
                                   ) 

df경상북도1= pd.read_csv('임상도/경상북도1.csv',
                 header=0,
                 usecols=['갱신년도','FRTP_CD','AGCLS_CD','DNST_CD','latitude','longitude','location'],                 
                 encoding='utf-8-sig'  # CSV 파일이 UTF-8 BOM 형식이라 utf-8-sig로 읽어야 함
                                   ) 

df경상북도2= pd.read_csv('임상도/경상북도2.csv',
                 header=0,
                 usecols=['갱신년도','FRTP_CD','AGCLS_CD','DNST_CD','latitude','longitude','location'],                 
                 encoding='utf-8-sig'  # CSV 파일이 UTF-8 BOM 형식이라 utf-8-sig로 읽어야 함
                                   ) 

df경상북도 = pd.concat( [df경상북도1 , df경상북도2 ], ignore_index=True )

df광주광역시= pd.read_csv('임상도/광주광역시.csv',
                 header=0,
                 usecols=['갱신년도','FRTP_CD','AGCLS_CD','DNST_CD','latitude','longitude','location'],                 
                 encoding='utf-8-sig'  # CSV 파일이 UTF-8 BOM 형식이라 utf-8-sig로 읽어야 함
                                   ) 

df대구광역시= pd.read_csv('임상도/대구광역시.csv',
                 header=0,
                 usecols=['갱신년도','FRTP_CD','AGCLS_CD','DNST_CD','latitude','longitude','location'],                 
                 encoding='utf-8-sig'  # CSV 파일이 UTF-8 BOM 형식이라 utf-8-sig로 읽어야 함
                                   ) 

df대전광역시= pd.read_csv('임상도/대전광역시.csv',
                 header=0,
                 usecols=['갱신년도','FRTP_CD','AGCLS_CD','DNST_CD','latitude','longitude','location'],                 
                 encoding='utf-8-sig'  # CSV 파일이 UTF-8 BOM 형식이라 utf-8-sig로 읽어야 함
                                   ) 

df부산광역시= pd.read_csv('임상도/부산광역시.csv',
                 header=0,
                 usecols=['갱신년도','FRTP_CD','AGCLS_CD','DNST_CD','latitude','longitude','location'],                 
                 encoding='utf-8-sig'  # CSV 파일이 UTF-8 BOM 형식이라 utf-8-sig로 읽어야 함
                                   ) 

df서울특별시= pd.read_csv('임상도/서울특별시.csv',
                 header=0,
                 usecols=['갱신년도','FRTP_CD','AGCLS_CD','DNST_CD','latitude','longitude','location'],                 
                 encoding='utf-8-sig'  # CSV 파일이 UTF-8 BOM 형식이라 utf-8-sig로 읽어야 함
                                   ) 

df세종특별자치시= pd.read_csv('임상도/세종특별자치시.csv',
                 header=0,
                 usecols=['갱신년도','FRTP_CD','AGCLS_CD','DNST_CD','latitude','longitude','location'],                 
                 encoding='utf-8-sig'  # CSV 파일이 UTF-8 BOM 형식이라 utf-8-sig로 읽어야 함
                                   ) 

df울산광역시= pd.read_csv('임상도/울산광역시.csv',
                 header=0,
                 usecols=['갱신년도','FRTP_CD','AGCLS_CD','DNST_CD','latitude','longitude','location'],                 
                 encoding='utf-8-sig'  # CSV 파일이 UTF-8 BOM 형식이라 utf-8-sig로 읽어야 함
                                   ) 

df인천광역시= pd.read_csv('임상도/인천광역시.csv',
                 header=0,
                 usecols=['갱신년도','FRTP_CD','AGCLS_CD','DNST_CD','latitude','longitude','location'],                 
                 encoding='utf-8-sig'  # CSV 파일이 UTF-8 BOM 형식이라 utf-8-sig로 읽어야 함
                                   ) 

df전라남도= pd.read_csv('임상도/전라남도.csv',
                 header=0,
                 usecols=['갱신년도','FRTP_CD','AGCLS_CD','DNST_CD','latitude','longitude','location'],                 
                 encoding='utf-8-sig'  # CSV 파일이 UTF-8 BOM 형식이라 utf-8-sig로 읽어야 함
                                   ) 

df전북특별자치도= pd.read_csv('임상도/전북특별자치도.csv',
                 header=0,
                 usecols=['갱신년도','FRTP_CD','AGCLS_CD','DNST_CD','latitude','longitude','location'],                 
                 encoding='utf-8-sig'  # CSV 파일이 UTF-8 BOM 형식이라 utf-8-sig로 읽어야 함
                                   ) 

df충청남도= pd.read_csv('임상도/충청남도.csv',
                 header=0,
                 usecols=['갱신년도','FRTP_CD','AGCLS_CD','DNST_CD','latitude','longitude','location'],                 
                 encoding='utf-8-sig'  # CSV 파일이 UTF-8 BOM 형식이라 utf-8-sig로 읽어야 함
                                   ) 

df충청북도= pd.read_csv('임상도/충청북도.csv',
                 header=0,
                 usecols=['갱신년도','FRTP_CD','AGCLS_CD','DNST_CD','latitude','longitude','location'],                 
                 encoding='utf-8-sig'  # CSV 파일이 UTF-8 BOM 형식이라 utf-8-sig로 읽어야 함
                                   ) 

df산불 = pd.read_csv('산불발생이력.csv',
                    header=0,
...
df전지역 = pd.concat( [df강원특별자치도 , df경기도, df경상남도, df경상북도, df광주광역시, 
                       df대구광역시, df대전광역시, df부산광역시,  df서울특별시, df세종특별자치시, 
                       df울산광역시 , df인천광역시 , df전라남도 , df전북특별자치도 , df충청남도 ,df충청북도], ignore_index=True )

newDf = df전지역.dropna( axis=0 )
a = newDf['갱신년도'] > 2019
b = newDf['갱신년도'] < 2025
new전지역 = newDf[a & b]

c = df산불['occu_year'] > 2019
d = df산불['occu_year'] < 2025
산불05 = df산불[c & d]

산림좌표 = new전지역.copy()
산불좌표 = 산불05.copy()

산림좌표['위도_소수3'] = 산림좌표['latitude'].round(3)
산림좌표['경도_소수3'] = 산림좌표['longitude'].round(3)

산불좌표['위도_소수3'] = 산불좌표['y'].round(3)
산불좌표['경도_소수3'] = 산불좌표['x'].round(3)
...
일치결과 = 같은좌표행[['FRTP_CD', 'AGCLS_CD', 'DNST_CD', 'ar']]

일치결과['ar'] = pd.to_numeric(일치결과['ar'], errors='coerce')
일치결과.loc[일치결과['ar'] == 0, 'ar'] = 1

일치결과 = 일치결과[일치결과['ar'] <= 10]
...
frtp통계 = 일치결과.groupby('FRTP_CD').agg({
    'ar': 'sum',
    'AGCLS_CD': 'count'
}).reset_index()

# 피해 면적을 발생 빈도로 나눈 값을 퍼센트로 계산
frtp통계['피해면적_빈도_비율'] = (frtp통계['ar'] / frtp통계['AGCLS_CD']) * 100
...
frtp통계['수종'] = frtp통계['FRTP_CD'].map(frtp이름)

print(frtp통계)
print(frtp통계[['수종', '피해면적_빈도_비율']])
...
# 수종 분석과 같은 일치결과 데이터를 사용해서 비교 기준을 맞춤
agcls통계 = 일치결과.groupby('AGCLS_CD').agg({
    'ar': 'sum',
    'FRTP_CD': 'count'
}).reset_index()

# 그래프에서 이해하기 쉽도록 발생 빈도 컬럼 이름 변경
agcls통계 = agcls통계.rename(columns={'FRTP_CD': '발생빈도'})

# 피해 면적을 발생 빈도로 나눈 값을 퍼센트로 계산
agcls통계['피해면적_빈도_비율'] = (agcls통계['ar'] / agcls통계['발생빈도']) * 100
...
agcls통계['수령'] = agcls통계['AGCLS_CD'].map(agcls이름)

print(agcls통계)
print(agcls통계[['수령', '피해면적_빈도_비율']])
...
ax1.set_ylabel('피해 면적 합계 / 발생 빈도')
ax1.set_xticks(x)
ax1.set_xticklabels(agcls통계['수령'], rotation=30)
